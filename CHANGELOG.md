...
- **M1 fix round 2**: recursively inspect executable default RHS expressions in object and array binding patterns used by declarations, function parameters and destructuring assignments, closing the remaining `fetch`, `Function` and `localStorage` laundering forms without changing runtime code or dependencies. A direct simple parameter default remains rejected, while matching defaults that resolve only to local bindings remain allowed
- M1 fix-round-2 TDD evidence — RED on `354542ebd3f28091b487d65d4588db8b42d24fc5`: focused `npx vitest run test/purity-policy.test.ts` reported 4 failed / 56 passed; the four newly confirmed object-declaration, nested-parameter, object-assignment and array-declaration bypasses returned no violation. GREEN: focused policy 60/60, complete `npm run test:pixi` 81/81, `npm run test:core` 71/71 and `npm run typecheck` clean for production and test TypeScript projects
