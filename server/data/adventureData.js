...
export const ADVENTURE_DEFINITIONS = [
  ...
  {
    id: "west_abandoned_road_watch",
    cityId: "west",
    category: "local_contract",
    title: "Abandoned Road",
    summary: "Patrol a disused caravan road outside Blackharbor where smugglers still cache gear along a route the city stopped maintaining.",
    riskBand: "Low",
    threatType: "Piercing ambush",
    recommendedPrep: ["Piercing reduction", "Quick Knife or short blade", "Field Bandage"],
    rewardCategory: "road salvage and rare caravan tokens",
    sourceLabel: "Old caravan road rumor",
    acquisition: ["Local Contract", "Blackharbor board", "Field Expedition"],
    opponent: foe("abandoned_road_scavenger", "Abandoned Road Scavenger", 2, 98, { strength: 12, defense: 9, speed: 13, dexterity: 14 }, "Piercing", "A scavenger picking through a caravan road that the city stopped patrolling years ago."),
    reward: { gold: 58, experience: 46, items: [{ itemId: "stolen_coin", quantity: 1 }] },
    extraDrops: [{ itemId: "rope_kit", quantity: 1 }],
    // Chance-based drop resolved server-side through the shared loot table (server/data/lootData.js),
    // reusing the same rollLoot() pattern combatService.js already uses for NPC loot families.
    lootFamily: "abandoned_road",
    // Pity protection: if worn_caravan_seal hasn't dropped in pityThreshold completed (won) runs,
    // the next win guarantees it. Tracked server-side in player.lootPity[adventure.id]; see adventureService.js.
    pityItemId: "worn_caravan_seal",
    pityThreshold: 6,
    gearHint: "Light piercing protection handles scavenger ambushes fine; the road itself is the real risk.",
  },
  {
    id: "west_smugglers_gate",
    cityId: "west",
    category: "hidden_site_run",
    title: "Smuggler's Gate",
    summary: "Present a worn caravan seal at a hidden gate to reach the smugglers' inner cache before the tide turns.",
    riskBand: "High",
    threatType: "Piercing and poison pressure",
    recommendedPrep: ["Blackharbor Corsair set", "Bitter Antidote", "Worn Caravan Seal"],
    rewardCategory: "smuggler cache gear and rare contraband",
    sourceLabel: "Smuggler's Gate rumor",
    acquisition: ["Item Requirement", "Hidden Site Discovery", "Black Market"],
    // Server-enforced item gate: see availability()/getRequiredItemStatus() in adventureService.js.
    // The item is consumed on a successful (won) run, not merely on attempt — see consumeRequiredItem().
    requiredItemId: "worn_caravan_seal",
    requiredItemQuantity: 1,
    requiredItemHint: "Rumored to be found along the Abandoned Road, a low-risk Blackharbor field contract.",
    chronicleMilestone: "Completed Smuggler's Gate and reached the smugglers' inner cache.",
    opponent: foe("smugglers_gate_warden", "Smuggler's Gate Warden", 4, 142, { strength: 16, defense: 14, speed: 15, dexterity: 16 }, "Piercing", "A gatekeeper who only opens the inner cache for those carrying the old caravan seal."),
    reward: { gold: 140, experience: 100, items: [{ itemId: "contraband_satchel", quantity: 1 }] },
    extraDrops: [{ itemId: "foreign_token", quantity: 1 }],
    gearHint: "Piercing and poison coverage matter once the gate opens; the caravan seal is what gets you through it.",
  },
  {
