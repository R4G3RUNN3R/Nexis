...
export function resolvePrestigeState(runtimeState, now = Date.now()) {
  ...
  const catalog = PRESTIGE_TITLE_DEFINITIONS.map((def) => ({ ...def, unlocked: titleIds.has(def.id) }));
  player.prestige = { currentTitleId, currentTitle: selected, titles, catalog, badges: badges.filter(Boolean), distinctions: [selected.label, ...badges.filter(Boolean).map((badge) => badge.label)].slice(0, 5), updatedAt: now };
