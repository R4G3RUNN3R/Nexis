...
function composeBoard(runtimeState, requestedCityId) {
  ...
  const front = contracts[0]
    ? entry("frontPage", { id: `front_${contracts[0].id}`, title: contracts[0].title, summary: contracts[0].summary, route: "/city#contracts", actionLabel: "Read contract", rewardLabel: contracts[0].reward?.gold ? `${contracts[0].reward.gold} gold` : "Local standing", requirementLabel: contracts[0].minimumStanding ? `${contracts[0].minimumStanding} standing` : "Open board", source: "local_contract" })
    : entry("frontPage", { id: `front_${cityId}_desk`, title: `${city.name} Desk Opens`, summary: "The local desk is accepting verified civic notices and paid leads.", route: "/city" });
  for (const contract of contracts) {
    const target = contract.combat?.opponentId ? sections.bounties : String(contract.type ?? "").toLowerCase().includes("civic") ? sections.civicAppointments : sections.opportunities;
    add(target, seen, entry(target === sections.bounties ? "bounties" : target === sections.civicAppointments ? "civicAppointments" : "opportunities", { id: `contract_${contract.id}`, title: contract.title, summary: contract.summary, route: "/city#contracts", actionLabel: "Open contract", rewardLabel: contract.reward?.gold ? `${contract.reward.gold} gold` : "Local reward", requirementLabel: contract.requirementLabel ?? (contract.minimumStanding ? `${contract.minimumStanding} standing` : "Local availability"), lockReason: standing < (contract.minimumStanding ?? 0) ? `Locked: requires ${contract.minimumStanding} ${city.name} standing.` : null, source: "local_contract" }));
  }
  ...
  for (const academy of academies.slice(0, 2)) {
    const required = asArray(academy.requiredCourses ?? academy.stages?.[0]?.requiredCourses);
    const missing = required.find((courseId) => !completed.has(courseId));
  ...
  for (const good of tradeGoods) {
    const missing = asArray(good.requiredCourses).find((courseId) => !completed.has(courseId));
  ...
  if (cityId === "west") add(sections.publicNotices, seen, entry("publicNotices", { id: "west_under_market", title: "Dock whispers mention under-market cargo", summary: "Some cargo does not reach the official ledger. Street Survival decides whether the rumor is useful or just bait.", route: "/black-market", actionLabel: "Check under-market", requirementLabel: "Street Survival", lockReason: lock(completed, "street-survival", "Street Survival"), source: "black_market" }));
...
export async function getCityBoardForUser(user, cityId) {
  return withTransaction(async (client) => { await createDefaultPlayerState(client, user.internalId); const playerState = await findPlayerStateByUserInternalId(client, user.internalId); if (!playerState) throw new HttpError(404, "Player state unavailable.", "PLAYER_STATE_NOT_FOUND"); const runtimeState = buildMutableRuntimeState(user, playerState); return { playerState, board: composeBoard(runtimeState, cityId) }; });
}
